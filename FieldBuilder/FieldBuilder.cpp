...
Field &FieldBuilder::createSecondLevel() {
    return createLevelFromFile("levels/secondLevel.txt");
}

Field& FieldBuilder::createFirstLevel() {
    return createLevelFromFile("levels/firstLevel.txt");
}

Field& FieldBuilder::createLevelFromFile(std::string path) {
        ...
        for(int j = 0; j < size_.x; j++) {
            ...
            switch (map[i][j]) {
                ...
                case 'p':
                    ...
                    break;
                default:
                    throw std::invalid_argument("wrong simbol in the map");
            }
        }
    }
